...
$n$개의 Seed Point가 있을 때 Voronoi Diagram은 $n + 1$개의 Face, 최대 $2n - 2$개의 Vertex, 최대 $3n - 3$개의 Edge를 갖는다.
...
- **Vertex**: 좌표값과 해당 Vertex에서 나가는 첫 번째 Edge(`firstEdge`) 포인터
...
평면 외곽 영역은 무한히 뻗어나가는 Unbounded Edge를 갖는다. Bounding Box와의 교점을 계산해 외곽 Edge를 유한한 영역으로 클리핑했다.
