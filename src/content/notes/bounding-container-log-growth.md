...
운영 장애 구간에서 한 백엔드 컨테이너의 애플리케이션 파일 로그는 약 9.4GB까지 증가했고,
...
사용하지 않는다. 정책 적용 효과는 같은 로그 경로를 정책 적용 전후 24시간 동안 측정한 자료로
별도로 확인해야 한다.
...
긴급 정리 명령은 과거 로그를 삭제하는 복구 수단이고, 회전과 보존 설정은 재발 방지 수단이다.
