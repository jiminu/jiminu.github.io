...
기하 알고리즘에서 좌표 계산(Geometry)과 연결 관계(Topology)가 섞이면 코드가 복잡해진다.

Winged-Edge를 사용하면 점의 위치 이동은 Vertex 좌표만 수정하면 되고, 다이어그램의 분할과 병합은 Edge의 포인터만 재연결하면 된다. 기하 연산과 위상 탐색의 책임을 분리할 수 있다.
